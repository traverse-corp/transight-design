...
컴포넌트, 스타일, 아이콘 registry 산출물을 갱신해야 할 때:
...
3. `npm run registry:build`를 실행합니다.
4. `npm run typecheck`를 실행합니다.
5. 문서 사이트에서 시각적으로 확인합니다.
...
4. `npm run registry:build`를 실행합니다.
...
4. `npm run registry:build`와 `npm run typecheck`를 실행합니다.
...
5. `npm run registry:build`를 실행합니다.
...
- 컴포넌트 설치 결과가 오래된 것 같으면 `registry:build`를 다시 실행합니다.
