...
React + TypeScript + Tailwind v4 + shadcn 초기화된 프로젝트. Vite와 Next.js 모두 지원.
...
# vite.config.ts, tsconfig.json에 @ alias 설정 (아래 §수동 셋업 참조)
...
cd my-app
# Next.js 템플릿은 Tailwind v4 + @ alias가 이미 셋업됨
npx shadcn@latest init --defaults
```
...
(`styles/index.css`가 SUIT 폰트 + Tailwind + 디자인 토큰 전부 import)

> ⚠️ Next.js에서 'use client' 이슈가 있을 수 있습니다. 우리 컴포넌트는 hook·이벤트 핸들러 사용 시 자체적으로 `'use client'` 선언이 들어가 있어 Server Component 환경에서도 정상 동작합니다.
...
- **Primary Blue** 3단 (`primary-blue-1`, `primary-blue-2`, `primary-blue-deep`) + **Skyblue** 2단 + 그래디언트
- **UI 9색 × 3단계** (red/orange/yellow/olive/green/skyblue/blue/purple/pink × text/base/pale)
- **Accent** (`accent-amber`) — 강조용 단일 색
...
신규 프로젝트에서 shadcn init 통과시키는 최소 셋업:
...
`create-next-app`이 대부분 자동 셋업합니다. 컴포넌트가 `src/components/`로 깔리는 환경을 위해 `components.json`에서 alias만 확인:
...
설치된 컴포넌트는 `src/components/base/` 또는 `src/components/custom/`로 떨어지므로 import는:

```tsx
import { Button } from '@/components/base/button'
```

**SUIT Variable 폰트 next/font 대체** — `app/layout.tsx`에서 SUIT @import 대신 Next 폰트 시스템을 쓰고 싶으면 (선택 사항, 현재는 CDN @import 방식):
```tsx
// 옵션이지만 CDN으로도 정상 동작하므로 굳이 안 바꿔도 됨
```
