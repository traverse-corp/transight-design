...
대표 ID:
```
ic-iaan-crime-type | ic-iaan-data-request | ic-iaan-freeze-request
ic-iaan-info-request | ic-iaan-open-management
ic-map-autotrace | ic-map-cluster | ic-map-crypto | ic-map-dollar | ic-map-won
ic-menu-analytics | ic-menu-dashboard | ic-menu-data-request | ic-menu-monitoring | ic-menu-txmap
ic-search-chain-type
ic-trace-bridge | ic-trace-ca | ic-trace-canvas | ic-trace-dt | ic-trace-search-history
```

현재 자체 Icon 풀은 Transight 도메인 시그니처 위주입니다. chevron, close, check,
search 돋보기 같은 범용 UI 액션은 `lucide-react`를 사용합니다.
...
- `primary-skyblue-1` / `-2`
- `ui-{red|orange|yellow|olive|green|skyblue|blue|purple|pink|amber}`
- `ui-text-{red|orange|...|pink|amber}` (더 어두운 텍스트 톤)
- `accent-amber` (@deprecated — `ui-amber`로 마이그레이션)
...
| 상황 | 시스템 | 호출 |
|------|------|------|
| 헤더 검색 입력 안 돋보기 | lucide | `<Search className="h-4 w-4 text-fg-muted" />` |
...
| `<Search className="h-4 w-4 text-gray-500" />` | `<Search className="h-4 w-4 text-fg-muted" />` (lucide도 색은 토큰만) |
