...
| 이름 | 역할 |
|---|---|
| `color` | 컬러 토큰 enum (12색). CSS 변수로 주입해 자식이 `var(--<comp>-active)` 등으로 참조하는 패턴을 권장합니다. |
...
| 12색 × N × M cva 매트릭스 정의 | `--<comp>-active` CSS 변수 + `color-mix()` 한 줄 |
